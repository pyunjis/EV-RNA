# Figure 6A Selective packaging heatmap_LV
...
cluster_heatmap <- function(Target, Baseline){
  ...
  # Check if DE genes include control (ERCC)
  spike <- rownames(cts_pos)[grep("^ENSG_", rownames(cts_pos))]
  length(spike)
  ...
  cts_all <- filtered_hd # counts with all fraction
  ...
  cts_HD_FR1619 <- cts_MM %>%
    ...
    mutate(HD_mean = mean(log2counts)) %>%
    mutate(delta = log2counts-HD_mean)
  cts_MM_FR1619 <- cts_MM %>%
    filter(Fraction == "FR1619", Condition == type2)
  cts_MM_FR1619$HD_mean <- cts_HD_FR1619$HD_mean
  cts_MM_FR1619$delta <- cts_MM_FR1619$log2counts - cts_HD_FR1619$HD_mean
  ...
  geneDF <- as.data.frame(gene_df[,59])
  geneDF$gene <- geneDF$`gene_df[, 59]`
  rownames(geneDF) <- geneDF$gene
  geneDF$`gene_df[, 59]` <- NULL
  test <- geneDF 
  ...
  clusterHeat2 <- pheatmap(plot_order2, show_rownames=F, 
                           ...
                           annotation_col = md_4, 
                           scale = "row", 
                           fontsize_row=4, fontsize_col=4, fontsize=8,
                           border_color=NA
  )
  save_pheatmap_pdf <- function(x, filename) {
    stopifnot(!missing(x))
    stopifnot(!missing(filename))
    pdf(filename, height = 4.5, width = 4.5)
    grid::grid.newpage()
    grid::grid.draw(x$gtable)
    dev.off()
  }
  ...
  save_pheatmap_pdf(clusterHeat2, paste0("./Figures/Selective packaging heatmap/Figure6A_protein_coding_", Target, "_", Baseline, ".pdf"))
