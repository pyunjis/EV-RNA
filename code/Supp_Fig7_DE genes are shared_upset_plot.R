# Upset plot of how many DE genes are shared
#library(VennDiagram) 
library(reshape2)
library(dplyr)
library(UpSetR)
...
#setwd("/Users/khyu/Box/EV-RNA/Main paper/4. Code/2021-06-21 V3/2021-06-21 V3_AK/Files/")
...
Upset <- function(target, set_type){
  ...
  pdf(paste0("./Figures/UpsetPlot_", target,".pdf",sep=""), paper = "a4r", width = 8, height = 5.5)
  #upset(fromList(gene_list), order.by = c("freq","degree"), decreasing = c(TRUE, TRUE), text.scale = 2)
  upset(fromList(gene_list),  sets = c("FR3033", "FR2326", "FR1619","FR912", "FR58", "FR14"), keep.order = TRUE,
        order.by = c("degree", "freq"), decreasing = c(TRUE, TRUE),  text.scale = 2)
...
Upset(target = "MM", set_type = set_MM) # pdf 8, 5.5
Upset(target = "LV", set_type = set_LV) # pdf 4, 5.5
Upset(target = "LG", set_type = set_LG) # pdf 8, 5.5
