rule fastqc:
    ...
    output:
        "samples/fastqc/191101/{sample}/{sample}_R1_fastqc.zip",
        "samples/fastqc/191101/{sample}/{sample}_R2_fastqc.zip"
    conda:
        "../envs/fastqc.yaml"
    shell:
        """fastqc --outdir  samples/fastqc/191101/{wildcards.sample} --extract  -f fastq {input.fwd} {input.rev}"""
...
rule fastqscreen:
    input:
        ...
        rev = "samples/raw/191101/{sample}_R2.fastq.gz"
    output:
        "samples/fastqscreen/191101/{sample}/{sample}_R1_screen.html",
        "samples/fastqscreen/191101/{sample}/{sample}_R1_screen.png",
        "samples/fastqscreen/191101/{sample}/{sample}_R1_screen.txt",
        "samples/fastqscreen/191101/{sample}/{sample}_R2_screen.html",
        "samples/fastqscreen/191101/{sample}/{sample}_R2_screen.png",
        "samples/fastqscreen/191101/{sample}/{sample}_R2_screen.txt",
    ...
    shell:
        """fastq_screen --aligner bowtie2 --conf {params.conf} --outdir samples/fastqscreen/191101/{wildcards.sample} {input.fwd} {input.rev}"""

rule multiqc:
    input:
        expand("samples/fastqc/191101/{sample}/{sample}_{ext}_fastqc.zip", sample = SAMPLES, ext = ext)
    output:
    ...
    conda:
        "../envs/multiqc.yaml"
    shell:
        """multiqc --outdir results/multiqc --filename {params.project_id}_multiqc samples/fastqc/191101"""
...
rule STAR:
    input:
        ...
        rev = "samples/raw/191101/{sample}_R2.fastq.gz"
    output:
        "samples/star/191101/{sample}_bam/Aligned.sortedByCoord.out.bam",
        "samples/star/191101/{sample}_bam/ReadsPerGene.out.tab",
        "samples/star/191101/{sample}_bam/Log.final.out"
    params:
        gtf=config["gtf_file"]
    run:
         ...
         shell("""
                ...
                --readFilesIn {input} \
                --outFileNamePrefix samples/star/191101/{wildcards.sample}_bam/ \
...
rule star_statistics:
    input:
        expand("samples/star/191101/{sample}_bam/Log.final.out",sample=SAMPLES)
    output:
...
rule htseq:
    input:
        "samples/star/191101/{sample}_bam/Aligned.sortedByCoord.out.bam"
    output:
        "samples/htseq/191101/{sample}_htseq_gene_count.txt"
    params:
        gtf = config["gtf_file"]
    conda:
        "../envs/htseq.yaml"
...
rule compile_counts:
    input:
        expand("samples/htseq/191101/{sample}_htseq_gene_count.txt",sample=SAMPLES)
    output:
...
rule read_distribution:
    ...
    params:
        bed=config['bed_file']
    output:
        "rseqc/read_distribution/191101/{sample}/{sample}.read_distribution.txt",
    conda:
        "../envs/rseqc.yaml"
    shell:
...
rule compile_rd:
    input:
        expand("rseqc/read_distribution/191101/{sample}/{sample}.read_distribution.txt", sample=SAMPLES)
    output:
